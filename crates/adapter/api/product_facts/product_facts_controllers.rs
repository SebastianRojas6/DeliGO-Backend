...
use application::mappers::api_mapper::{ApiOutMapper, ApiInMapper};
...
use application::usecases::product_fact_uc::update_product_fact_usecase::UpdateProductFactUseCase;

pub fn routes(cfg: &mut ServiceConfig) {
    cfg.service(get_all_products_facts)
        .service(get_one_product_fact_by_id)
        .service(create_product_fact)
        .service(update_product_fact)
        .service(delete_product_fact_by_id);
}
...
async fn get_all_products_facts(data: web::Data<AppState>) -> Result<HttpResponse, ErrorResponse> {
    ...
    product_facts
        ...
        .map(|facts| HttpResponse::Ok().json(facts.into_iter().map(ProductFactPresenterMapper::to_presenter).collect::<Vec<ProductFactPresenter>>()))
}
...
async fn get_one_product_fact_by_id(data: web::Data<AppState>, fact_id: web::Path<i32>) -> Result<HttpResponse, ErrorResponse> {
    ...
    product_fact
        .map_err(ErrorResponse::map_io_error)
        .map(|fact| HttpResponse::Ok().json(ProductFactPresenterMapper::to_presenter(fact)))
}
